...
{staffs.map((item,i) => (
     <Component key={i} variant>
            ...
            <Button ref ={aRef}
              onClick={() =>( getData(item.image, item.heading, item.description, item.qualities,)) }
            >
              More Info
            </Button>
	 </Component>
  ...
  /> */}
  {
	  showDialog === true ? <CardModal image={tempData[1]} heading={tempData[2]} description={tempData[3]} qualities={tempData[4]} showDialog={showDialog}
	  setShowDialog={setShowDialog} /> : null
  }
